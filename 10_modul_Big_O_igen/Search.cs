namespace SearchMethods
{
    public class Search
    {
        ...
        public static int FindNumberBinary(int[] array, int tal) {
            ...
            while (min <= max) { // Så længe min er mindre end eller lig med max
                int mid = (min + max) / 2; // Find midten af arrayet
                if (tal == array[mid]) { // Hvis tallet er midt i arrayet
                    return mid; // Returner indekset
                } else if (tal < array[mid]) { // Hvis tallet er mindre end midten
                    max = mid - 1; // Sæt max til midten - 1
                } else { // Hvis tallet er større end midten
